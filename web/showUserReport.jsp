...
<%@page import="com.thirdchimpanzee.myhelpdesksupport.service.QuestionService"%>
<%@page import="com.thirdchimpanzee.myhelpdesksupport.entity.HelpDeskUser"%>
<%@page import="com.thirdchimpanzee.myhelpdesksupport.entity.HelpDeskQuestion"%>
...
HelpDeskUser _HDU = com.thirdchimpanzee.myhelpdesksupport.service.UserService.getInstance().findUserById(agentId);
if (_HDU != null) {
	List<HelpDeskQuestion> _results = QuestionService.getInstance().getAllHelpDeskQuestionsByOrgAndAgent(String.valueOf(_HDU.getOrgId()),String.valueOf(_HDU.getId()));
	for (HelpDeskQuestion helpDeskQuestion : _results) {
%>
<tr width="100%">
						 	<td width='10%'><%=helpDeskQuestion.getId()%></td>
						 	<td width='20%'><%=helpDeskQuestion.getQuestion()%></td>
						 	<td width='30%'>Raised by <%=helpDeskQuestion.getAskedBy()%> on <%=helpDeskQuestion.getAskedDateTime()%></td>
						 	<td width='20%'><%=helpDeskQuestion.getAnswer()%></td>
						 	<td width='10%'><%=helpDeskQuestion.getRating()%></td>
						 	<td width='20%'><%=helpDeskQuestion.getStatus()%></td>
</tr>
<%		
	}
}
